...
const content = fs.readFileSync('C:/P99/Logs/eqlog_Inthebot_P1999Green.txt', 'utf8')
var order = 0
var tempcount = 0
var tempString = ''
var chain = []
...
client.on('ready', async () => {
    ...
    await client.channels.cache.get('1085218184921104466').bulkDelete(10)

    //client.channels.cache.get('1085218184921104466').send('test')
    //console.log(process.cwd())
    chainForDiscord = instructions + '---- Reporting Current Chain ----\n'
    await client.channels.cache.get('1085218184921104466').send(chainForDiscord).then(sent => {
        lastMsgID = sent.id
        //console.log(lastMsgID)
    })
    //do {
    client.setInterval(processCommands(), 2000) // Process commands every 2 seconds
    //} while (commandsToProcess.length)
})


fs.watch('C:/P99/Logs/eqlog_Inthebot_P1999Green.txt', (eventType, filename) => {
    readLastLines.read('C:/P99/Logs/eqlog_Inthebot_P1999Green.txt', 1)
    .then(async (lastLine) => {
        if(lastLine.includes('!chain')){
            commandsToProcess.push(lastLine.trim())
            console.log(commandsToProcess)
        }
    })
})
...
/*
    ...
    if (commandsToProcess.includes('start')){
        console.log(commandsToProcess.trim())
        ...
        client.channels.cache.get('1085218184921104466').bulkDelete(5)
        client.channels.cache.get('1085218184921104466').send(chainForDiscord).then(sent => {
        lastMsgID = sent.id
        //console.log(lastMsgID)
        })
    }
    ...
    if (commandsToProcess.includes('add')){
        console.log(commandsToProcess.trim())
        tempString = commandsToProcess.toUpperCase().trim().split(/[ ']+/g).slice(-2).slice(0,1).toString()
    ...
    if (commandsToProcess.includes('remove')){
        console.log(commandsToProcess.trim())
        tempString = commandsToProcess.toUpperCase().trim().split(/[ ']+/g).slice(-2).slice(0,1).toString()
        console.log(tempString)
        if(chain.includes(tempString)){
    ...
    if (commandsToProcess.includes('report')){
        console.log(commandsToProcess.trim())
        index = 0
